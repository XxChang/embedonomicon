...
`std` crate 是Rust的标准库。它包含的功能需要假设程序将运行在一个操作系统上而不是[*直接运行在裸机*]上。`std` 也假设操作系统是一个通用的操作系统，像是会在服务器和桌面看到的那些系统。因此，`std` 在那些经常能在操作系统中的看到的功能:线程，文件，套接字，一个文件系统，进程，等等之上，提供一个标准的API 。

[*直接运行在裸机上*]: https://en.wikipedia.org/wiki/Bare_machine

换句话说，`core` crate是`std` crate的一个子集，其不对将运行程序的系统做任何假设。它提供与语言的基本类型，像是浮点数，字符串和切片有关的APIs，也提供像是原子操作和SIMD指令这样的处理器功能的APIs 。然而它缺少涉及到堆内存分配和I/O有关的APIs。
...
如果你配置的不是在恐慌时无条件终止，大多数与完整的操作系统有关的目标都不是(或者如果你的 [自制目标][custom-target] 不包含
...
另外，声明 `eh_personality` 函数。一个简单的没有什么特别的东西，展开时和下面一样:
